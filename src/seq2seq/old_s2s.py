...
class Decoder(nn.Module):
    def __init__(self, input_size, context_size, out_features):
        ...
        self.rnn = nn.GRU(
            input_size=input_size, hidden_size=context_size, batch_first=True
        )
...
class Seq2SeqModel(pl.LightningModule):
    def __init__(self, input_vocab_size: int, output_vocab_size: int):
        ...
        self.decoder = Decoder(
            input_size=10, context_size=1024, out_features=output_vocab_size
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        start_token = (
            torch.LongTensor(
                [self.textual_tokenizer.tok2idx[self.textual_tokenizer.SOS_TOKEN]]
            )
            .unsqueeze(0)
            .cuda()
        )
    ...
    def train_dataloader(self):
        ...
        dset = Seq2SeqDataset(
            ...
            df["numerical_tokenized"].tolist(),
            self.textual_tokenizer,
            df["textual_tokenized"].tolist(),
        )
...
if __name__ == "__main__":
    ...
    trainer = pl.Trainer(
        gpus=[0], gradient_clip=1.0, accumulate_grad_batches=32, max_epochs=10000
    )
    trainer.fit(model)
